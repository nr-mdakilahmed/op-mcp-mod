...
class GoogleOAuthHandler:
    ...
    def _initialize_flow(self):
        ...
        try:
            ...
            self.flow = Flow.from_client_config(client_config, scopes=["openid", "email", "profile"])
            self.flow.redirect_uri = self.config.GOOGLE_REDIRECT_URI

            logger.info(
                "Google OAuth flow initialized",
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Google OAuth configuration error"
            ) from e
    ...
    async def handle_callback(self, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            id_info = google.oauth2.id_token.verify_oauth2_token(
                id_token=credentials.id_token, request=request_adapter, audience=self.config.GOOGLE_CLIENT_ID
            )
            ...
            if self.config.oauth_allowed_domains_list:
                ...
                if domain not in self.config.oauth_allowed_domains_list:
                    logger.warning(
                        "OAuth login attempt from unauthorized domain",
                        email=email,
                        domain=domain,
                        allowed_domains=self.config.oauth_allowed_domains_list,
                    )
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN, detail=f"Email domain '{domain}' is not authorized"
                    )

            # Require verified email
            if not user_info.get("email_verified"):
                raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Email address must be verified")

            logger.info("Google OAuth login successful", email=user_info.get("email"), name=user_info.get("name"))
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Authentication error") from e
