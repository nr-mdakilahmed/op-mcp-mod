...
class ToolResponse(BaseModel):
    ...
    success: bool
    result: Optional[List[ContentItem]] = None
...
def create_remote_app() -> FastAPI:
    ...
    async def login(auth_request: AuthRequest, config: Config = Depends(Config.from_env)):
        ...
        if auth_request.username == "admin" and auth_request.password == "admin":
            access_token = create_access_token(
                data={"sub": auth_request.username, "scopes": ["read", "write"]},
                config=config
            )
    ...
    async def execute_tool(
        ...
        try:
            ...
            if isinstance(result, (list, tuple)):
                for item in result:
                    # For now, we'll simplify and just convert everything to string
                    # This ensures compatibility with different MCP content types
    ...
    async def websocket_endpoint(websocket: WebSocket):
        ...
        except (ConnectionError, ValueError, TypeError, asyncio.TimeoutError, RuntimeError) as e:
